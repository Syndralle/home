...
def boar_brawl(player_score, opponent_score):
    ...
    diff_abs = abs(ones_digit - tens_digit)
    if diff_abs >= 1:
        return 3 * diff_abs
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    base = strategy(0, 0)
    for s0 in range(0, goal):
        for s1 in range(0, goal):
            if base != strategy(s0, s1):
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_index = 1
    max_val = 0
    func = make_averaged(roll_dice, samples_count)
    for _ in range(1, 10 + 1):
        contrast = func(_, dice)
        if contrast > max_val:
            max_index = _
            max_val = contrast
    return max_index
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_update = sus_update(0, score, opponent_score)
    if score_update - score >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
